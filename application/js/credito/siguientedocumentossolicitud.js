$(document).on("ready", function(){
	...
	$("#siguientedocumentossolicitud").click(registracliente);
	//$("#siguientedocumentossolicitud").click(displaydocumentossolicitados);

	$("#nextencuesta").click( clienteregistradoorgnext );
});


function registracliente(){
		
	url = now + "index.php/api/clienteorgrest/insertclienteorgfisico/format/json/";
	$.post(url, $("#clienteindb").serialize() ).done(function(data){

		
		
		if (data!=0) {

					$("#clienteregistradoorg").val(data);
					clienteregistradoorgnext();					
		}else{
...
function clienteregistradoorgnext(){

	cliente = $("#clienteregistradoorg").val();

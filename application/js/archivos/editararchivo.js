...
function actualizaomision(e){
	//alert(e.target.id);
	iddocumentosolicitado = e.target.id;

	url = now + "index.php/api/solicituddocumentosrest/actualizaOmision/format/json/";

	$.post(url , {"iddocumentosolicitado" : iddocumentosolicitado }).done(function(data){
		listdocumentsbyarchivo();
	}).fail(function(){
